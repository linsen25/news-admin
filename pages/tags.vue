  ...
  <div>
    ...
    <section class="panel taxonomy-section">
      ...
      <form v-if="canManage" class="taxonomy-form category-form" @submit.prevent="saveCategory">
        <label>中文名称<input v-model="categoryDraft.name" required maxlength="50" placeholder="例如：加拿大"></label>
        <label>英文名称<input v-model="categoryDraft.nameEn" required maxlength="50" placeholder="例如：Canada"></label>
      ...
      <div class="category-tree">
        <article v-for="category in categories" :key="category.id" class="category-row">
          <div><strong>{{ category.name }}</strong><small>{{ category.nameEn || '尚未填写英文名称' }}</small></div>
    ...
    <section class="panel taxonomy-section">
      <div class="taxonomy-heading"><div><p class="eyebrow">TAGS</p><h2>标签管理</h2></div><span>标签从属于一个分类</span></div>
      <form v-if="canManage" class="taxonomy-form tag-form" @submit.prevent="saveTag">
        <label>中文名称<input v-model="tagDraft.name" required maxlength="50" placeholder="例如：移民政策"></label>
        <label>英文名称<input v-model="tagDraft.nameEn" required maxlength="50" placeholder="例如：Immigration Policy"></label>
      ...
      <div class="tag-table">
        <div class="tag-table-head"><span>中英文名称</span><span>所属分类 / 网址标识</span><span>操作</span></div>
        <div v-for="tag in tags" :key="tag.id" class="tag-table-row"><span class="bilingual-name"><strong>{{ tag.name }}</strong><small>{{ tag.nameEn || '尚未填写英文名称' }}</small></span><span class="tag-category"><b>{{ categoryName(tag.categoryId) }}</b><code>{{ tag.slug }}</code></span><div class="row-actions"><button type="button" @click="editTag(tag)">{{ canManage ? '编辑' : '🔒 编辑' }}</button><button class="reject" type="button" @click="removeTag(tag)">{{ canManage ? '删除' : '🔒 删除' }}</button></div></div>
...
const categoryDraft = reactive({ name: '', nameEn: '', slug: '' });
const tagEditingId = ref(''); const tagSaving = ref(false);
const tagDraft = reactive({ name: '', nameEn: '', slug: '', categoryId: '' });
...
const resetCategory = () => { categoryEditingId.value = ''; Object.assign(categoryDraft, { name: '', nameEn: '', slug: '' }); };
const editCategory = (item: CatalogItem) => { if (!canManage.value) return locked('编辑分类'); categoryEditingId.value = item.id; Object.assign(categoryDraft, { name: item.name, nameEn: item.nameEn, slug: item.slug }); };
const saveCategory = async () => { categorySaving.value = true; try { const input = { name: categoryDraft.name.trim(), nameEn: categoryDraft.nameEn.trim(), slug: categoryDraft.slug.trim().toLowerCase() || autoSlug(categoryDraft.nameEn, 'category'), parentId: null }; if (categoryEditingId.value) await api.updateCategory(categoryEditingId.value, input); else await api.createCategory(input); toast.success(categoryEditingId.value ? '分类修改成功' : '分类创建成功'); resetCategory(); await refreshCategories(); } catch (error) { toast.error(getApiErrorMessage(error)); } finally { categorySaving.value = false; } };
...
const resetTag = () => { tagEditingId.value = ''; Object.assign(tagDraft, { name: '', nameEn: '', slug: '', categoryId: categories.value[0]?.id || '' }); };
const editTag = (item: CatalogItem) => { if (!canManage.value) return locked('编辑标签'); tagEditingId.value = item.id; Object.assign(tagDraft, { name: item.name, nameEn: item.nameEn, slug: item.slug, categoryId: item.categoryId || '' }); };
const saveTag = async () => { tagSaving.value = true; try { const input = { name: tagDraft.name.trim(), nameEn: tagDraft.nameEn.trim(), slug: tagDraft.slug.trim().toLowerCase() || autoSlug(tagDraft.nameEn, 'tag'), categoryId: tagDraft.categoryId }; if (tagEditingId.value) await api.updateTag(tagEditingId.value, input); else await api.createTag(input); toast.success(tagEditingId.value ? '标签修改成功' : '标签创建成功'); resetTag(); await refreshTags(); } catch (error) { toast.error(getApiErrorMessage(error)); } finally { tagSaving.value = false; } };
